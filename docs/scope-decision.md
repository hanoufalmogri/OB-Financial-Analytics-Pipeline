...
## Addendum: recurring payment detection is a low-confidence heuristic, not a finished subscription list

`recurring_payments` groups transactions by `card_id` + `merchant_id` + similar amount, and checks whether they repeat at a regular interval (see the model's own logic for the exact thresholds). The 47 series it found should be read as **low-confidence candidates**, not a finished list of real subscriptions.

The core limitation: the dataset only has `merchant_id` (an opaque numeric ID) and a location, never a merchant *name* or *description*. Amount-and-interval matching alone can't distinguish "this is genuinely the same recurring subscription" from "this person coincidentally bought something similarly priced at the same shop on a similar cadence" — e.g. a $3.67 charge at the same coffee shop every week reads identically to a genuine $3.67/week subscription under this logic, even though it's far more likely to just be someone's regular coffee habit. Consistent with that read: every one of the 47 detected series sits at exactly 3 occurrences (the minimum allowed) and several are small, weekly, low-dollar amounts — the profile of repeat everyday purchases, not typical subscription behavor.

**What would improve this:** merchant name or description data (e.g. "Netflix", "Planet Fitness") would let the model corroborate the amount/interval pattern against a real business identity, turning a purely statistical coincidence-detector into something that could distinguish a coffee habit from an actual subscription. That data doesn't exist in this dataset, so it's a known ceiling on this feature, not something the current pipeline can resolve.
